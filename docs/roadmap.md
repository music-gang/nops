...
| `multi-namespace` | One instance manages the namespaces of `-nomad-namespaces` (replaces `-nomad-namespace`): a job is `(namespace, ID)`, its namespace is the one its HCL declares, hooks live in their job's namespace, `nomadx` takes the namespace per call; a job in an unlisted namespace is an ERROR ([configuration](configuration.md#namespaces), [design](design/engine-detection.md#namespaces), [decisions](design/decisions.md) 2026-09-25, #37) | `internal/config`, `internal/nomadx`, `internal/hooks`, `internal/engine`, `cmd/nops` |
